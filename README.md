...
* Routing, which is used to specify the request handler and middleware needed based upon the URL being visited and the HTTP method used.

* Helpers, providing optional functionality out-of-the-box to ease the writing of api servers and web apps. (e.g. `.status` or `.end` for response objects)

- [x] Describe Middleware?

* Middleware are functions that can be chained together to dynamically act upon requests by potentially changing the request/response objects and either passing onto the next middleware (which could be an error handler or regular middleware) via `next()` or ending the request/response cycle by returning a status and optional resource.

- [x] Describe a Resource?

* A resource is a piece or set of data/information that a client may want to access or manipulate.

- [x] What can the API return to help clients know if a request was successful?

* The API can return a status code. For instance, 201 for the successful creation of a resource or 403 if the request failed because the client (w/ a known identity) isn't authorized to make the request. If you're rejecting the request (e.g. 400, 401, 403, 404), it would also be helpful to include a description of the issue in a JSON object.

- [x] How can we partition our application into sub-applications?

* Express Routers allow us to partition into sub-applications that we can then treat in a more modular fashion, with middleware specific to each sub-application specified within a file specific to that router. We can then import those routers into a central router, such as our `server.js` file.
